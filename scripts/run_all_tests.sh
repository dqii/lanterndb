...
# create the folder
# Add error handling to ensure that the tests are not run if the downloads fail
...
then
    ...
    pushd $TMP_ROOT/vector_datasets
        curl -sSo sift_base1k.csv https://storage.googleapis.com/lanterndb/sift_base1k.csv || { echo "Failed to download sift_base1k.csv"; exit 1; }
        curl -sSo siftsmall_base.csv https://storage.googleapis.com/lanterndata/siftsmall/siftsmall_base.csv || { echo "Failed to download siftsmall_base.csv"; exit 1; }
        curl -sSo tsv_wiki_sample.csv https://storage.googleapis.com/lanterndb/tsv_wiki_sample.csv || { echo "Failed to download tsv_wiki_sample.csv"; exit 1; }
...
# Add an option to run a subset of the tests or to run the tests in a specific order
# This can be done by adding a command line argument to the script that specifies the tests to run
# The argument can be a list of test names or a pattern that matches the test names
# If the argument is not provided, the script will run all the tests as before
TESTS="${1:-*}"

# Add comments to explain how the schedule is generated and how it can be modified
# The schedule is generated by reading the `schedule.txt` file and selecting the tests that match the `TESTS` argument
# The order of the tests in the schedule is the order in which they will be run
# To change the order of the tests, simply rearrange the lines in the `schedule.txt` file
# To add a new test, add a line to the `schedule.txt` file with the name of the test
# To remove a test, delete the line from the `schedule.txt` file or comment it out by adding a `#` at the beginning of the line

# Add comments to explain what each part of the script does
# This script runs the tests for the lanterndb extension. It downloads necessary files for the tests, generates a schedule of tests to run, and then runs the tests using the `pg_regress` tool. The results of the tests are saved in the `TMP_OUTDIR` directory.
fi
